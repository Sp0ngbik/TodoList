...
export const fetchTasks = createAsyncThunk('tasks/getTasks', async (todoListId: string, thunkAPI) => {
    try {
...
export const fetchCreateTask = createAsyncThunk('tasks/createTask', async (arg: { todoListId: string, title: string }, thunkAPI) => {
...
export const fetchDeleteTask = createAsyncThunk('tasks/deleteTask', async (arg: {
    todoListId: string,
    taskId: string
}, {dispatch, rejectWithValue}) => {
    dispatch(appSetStatusAC({status: 'loading'}))
    try {
        dispatch(changeTaskEntityStatusAC({todoListId: arg.todoListId, taskId: arg.taskId, status: 'loading'}))
...
export const fetchUpdateTaskField = createAsyncThunk('tasks/updateField', async (arg: { todoListId: string, taskId: string, newField: T_PutTask }, thunkAPI) => {
    ...
    if (task) {
        ...
        try {
            ...
            if (newTask.data.resultCode) {
                ...
                return {todolisId: arg.todoListId, taskId: arg.taskId, taskModel: arg.newField}
            }
        } catch (e) {
            networkErrorHandler(thunkAPI.dispatch, e)
            return thunkAPI.rejectWithValue(null)
...
export const taskSlice = createSlice({
    ...
    initialState,
    reducers: {
        changeTaskEntityStatusAC: (state, action: PayloadAction<{ todoListId: string, taskId: string, status: T_ResponseStatus }>) => {
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(fetchUpdateTaskField.fulfilled, (state, action) => {
                ...
                state[action.payload.todolisId][task] = {...state[action.payload.todolisId][task], ...action.payload.taskModel}
            })
    }
